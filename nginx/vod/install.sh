...
http {
  ...
  # Public routes rewrite MPEG-TS segment names to disguised .jpeg URLs.
  # Keep this explicit: switching to fMP4 would require .m4s + init routes.
...
server {
  ...
  location ~ ^/([^/]+\.json)/playlist\.m3u8$ {
    ...
    sub_filter '/index-v1.m3u8' '/video.m3u8';
    sub_filter '/index-a1.m3u8' '/audio.m3u8';
    sub_filter_once off;
  }
  ...
  location ~ ^/([^/]+)/master\.m3u8$ {
    ...
    sub_filter '.json/index-v1.m3u8' '/video.m3u8';
    sub_filter '.json/index-a1.m3u8' '/audio.m3u8';
    sub_filter_once off;
  }

  # Pattern 5: friendly media playlist. index.m3u8 lets nginx-vod select the
  # available muxed (v1-a1), video-only (v1), or audio-only (a1) tracks.
  location ~ ^/([^/]+)/(video|audio)\.m3u8$ {
    if ($request_method = OPTIONS) {
      add_header Access-Control-Allow-Origin * always;
      add_header Access-Control-Allow-Headers '*' always;
      add_header Access-Control-Allow-Methods 'GET, HEAD, OPTIONS' always;
    ...
    proxy_set_header Host $host;
    proxy_set_header Accept-Encoding "";
    proxy_pass http://127.0.0.1:8889/hls/$1.json/index.m3u8;
    ...
    sub_filter '-v1.ts' '-v1.jpeg';
    sub_filter '-a1.ts' '-a1.jpeg';
    sub_filter_once off;
  }

  # Pattern 6: legacy muxed segment.
  # /xxx/v-2.jpeg -> /hls/xxx.json/seg-2-v1-a1.ts
  location ~ ^/([^/]+)/v-(\d+)\.jpeg$ {
    limit_rate 3m;
  ...
  # Pattern 7: separated video/audio segment.
  # /xxx/v-2-v1.jpeg -> /hls/xxx.json/seg-2-v1.ts
  # /xxx/v-2-a1.jpeg -> /hls/xxx.json/seg-2-a1.ts
  location ~ ^/([^/]+)/v-(\d+)-(v1|a1)\.jpeg$ {
    limit_rate 3m;
    limit_rate_after 100k;
    ...
    proxy_set_header Connection "";
    proxy_set_header Host $host;
    proxy_pass http://127.0.0.1:8889/hls/$1.json/seg-$2-$3.ts;
  ...
  # Pattern 8: redirect media playlists cached before the disguised .jpeg
  # mapping was deployed, so every separated segment ends at the same route.
  # /xxx/v-2-v1.ts -> /xxx/v-2-v1.jpeg
  # /xxx/v-2-a1.ts -> /xxx/v-2-a1.jpeg
  location ~ ^/([^/]+)/v-(\d+)-(v1|a1)\.ts$ {
    if ($request_method = OPTIONS) {
      add_header Access-Control-Allow-Origin * always;
      add_header Access-Control-Allow-Headers '*' always;
      add_header Access-Control-Allow-Methods 'GET, HEAD, OPTIONS' always;
    ...
    return 302 /$1/v-$2-$3.jpeg;
  }

  # Pattern 9: Thumbnail proxy
